...
**注意：这里只是增加了一个语音识别的新词条，在识别后并无任何执行动作。如需加入识别后的动作，请详细参考源码程序注释。**
...
<img src="../_static/media/11.voice_interaction_and_intelligent_handling_course/2.1/image16.png" style="width:700px" />
...
**注意：在键盘输入代码位置序号后按下“Shift+G”键，即可直接跳转到对应位置。（本节旨在介绍快速跳转方法，故代码位置序号仅供参考，请以实际为准。）**
...
| **控制方式** |           命令行启动玩法后           |                        |
|              |               语音控制               |        自动工作        |
| **应用场景** | 安静环境（语音指令有效距离小于30cm） | 嘈杂环境（无距离要求） |
| **工作模式** |               单次搬运               |        连续搬运        |
...
4)  程序中设置编号为1的词条为唤醒词条，若修改了编号为1的词条内容即修改了
...
1)  找到代码“**tts.TTSModuleSpeak('\[h0\]\[v10\]\[m3\]', '准备就绪')**”，程序默认播放内容是“**准备就绪**”，如下图所示：
...
5)  当机器人识别到颜色物体，通过cv2.drawContours()函数来绘制颜色物体的轮廓，如下所示：
...
6)  机器人识别到颜色物体后，同时用cv2.circle()函数在回传画面内绘制颜色物体的中心点，如下所示：
