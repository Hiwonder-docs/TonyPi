...
<img src="../_static/media/9.upper_computer_action_editing_course/1.1/image1.png" style="width:80px" />
...
注意：动作组序号请勿选用已定义序号和255号，动作组名称请与存储目录**（/home/pi/TonyPi/ActionGroups）**内的名称保持一致。
...
<img src="../_static/media/9.upper_computer_action_editing_course/5.1/image8.png" style="width:300px" />

动作名称可以任意填写，但动作组编号要填写正确，否则无法实现。

4. 添加完成后，单击该动作名称按钮便可以执行一次。

<img src="../_static/media/9.upper_computer_action_editing_course/5.1/image9.png" style="width:250px" />
...
**注意：动作组文件必须保存在目录“/home/pi/TonyPi/ActionGroups”下。**
...
**注意！动作组文件必须保存在目录“/home/pi/TonyPi/ActionGroups”下，若想调用自定义动作组，可参照“第9章 上位机动作编辑课程\第3课 动作编辑教学”来编辑动作。**
...
**以下操作是基于“3.1[调用单个动作组](#_调用单个动作组)”基础上继续操作。**

用户可以通过复制粘贴多行调用代码的方法，令机器人**依次执行多个动作组**，本节以调用“**go**”、“**move_up**”动作组为例，具体操作步骤如下：

1)  参照“[**3.1 调用单个动作组**](#_调用单个动作组)”的步骤1-3，打开程序文件(不要进入编辑模式，否则会复制失败，若当前是编辑模式，可按下键盘上的“Esc”键)。
...
**注意：动作组文件必须保存在目录“/home/pi/ArmPi_mini/ActionGroups”下，且程序内的动作组名称要与文件夹中存储的名称一致，否则将调用失败！**
